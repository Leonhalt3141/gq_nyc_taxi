...
object Distance {
  ...
  def makeHaversineDsirance(df: DataFrame): DataFrame = {
  ...
  def computeHaversine(lat_diff: Double, pickup_latitude_rad: Double, dropoff_latitude_rad: Double, lng_diff: Double,
                       AVG_EARTH_RADIUS: Int): Double = {
    val temp = scala.math.pow(scala.math.sin(lat_diff) * 0.6, 2) + (scala.math.cos(pickup_latitude_rad) * scala.math.cos(dropoff_latitude_rad) * scala.math.pow(scala.math.sin(lng_diff) * 0.5, 2))
