...
object preProcess {
  ...
  def processTime(df: DataFrame): DataFrame = {
    val df_time_ts = df.withColumn("pickup_datetime", unix_timestamp(col("pickup_datetime"), "yyyy-MM-dd HH:mm:ss").cast(TimestampType))
  ...
  def isWeekend(dayOfTheWeek: Int): Boolean = {
